...
do
	...
	function MergedInput:axis(id)
		local value = 0
		for i=1, #self._handlers do
			value = math.max(value, self._handlers[i]:axis(id))
		end
	...
	function MergedInput:reset(id)
		for i=1, #self._handlers do
			self._handlers[i]:reset(id)
		end
	end

	function MergedInput:releaseInput(id)
		for i=1, #self._handlers do
			self._handlers[i]:releaseInput(id)
		end
	end

	function MergedInput:endFrame(dt)
		for i=1, #self._handlers do
			self._handlers[i]:endFrame(dt)
		end
	end
